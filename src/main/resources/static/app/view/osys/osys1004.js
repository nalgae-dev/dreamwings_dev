Ext.define('DreamNalgae.view.osys.osys1004', {
        ...
        onAfterRender: function(panel) {
            ...
            Ext.Ajax.request({
                ...
                success: function(res) {
                    ...
                    //panel.lookupReference('partnerGrid').getStore().loadData(data.partners);
                    console.log(data);
                },
                failure: function() {
                    Ext.Msg.alert('Error','초기 데이터를 불러오는 데 실패했습니다.');
                }
            });
        },
        excel: function(btn) {
            const view = btn.up('osys1004');
            const grid = view.lookReference('partnerGrid');
            const store = grid.getStore();
            const columns = grid.getVisibleColumns().filter(col => col.dataIndex);
            const data = [];
            ...
            store.each(function(record) {
                const row = columns.map(col => record.get(col.dataIndex));
            });

            const worksheet = XLSX.utils.aoa_to_shet(data);
            const workbook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, worksheet, '거래목록');

            XLSX.writeFile(workbook, '거래처목록.xlsx');
        }
    ...
    items:[
        {
            ...
            tbar:[
                ...
                { xtype: 'button', text: '엑셀', iconCls: 'x-fa fa-file-excel', handler: 'excel' },
                ...
                // 1. 검색 조건 영역
                {
                    ...
                    items: [
                        ...
                        { 
                            ...
                            //                 Ext.Msg.alert('에러', '출고팀 목록을 불러오지 못했습니다.');
                            //             }
                            //         }
                            //     }
                            // },
